...
class scMO():
    ...
    def id_mapping(self):
      mappings = pd.read_excel("Data/prot_map.xlsx")
...
def cell_embedding_svgae(scmo, args):
  ...
  adj_matrix_p = torch.tensor(scmo.prot.obsm["cell_similarity_top"])
  adj_matrix_p.fill_diagonal_(1)

  adj_matrix_r = torch.tensor(scmo.rna.obsm["cell_similarity_top"])
...
def cell_embedding_vgae(scmo, args):
  ...
  if args.use_embeddings:
    if "embeddings" not in scmo.rna.obsm:
      scmo.rna.obsm["embeddings"] = rna_embedding_pca(scmo.rna)
    x_data_rna = scmo.rna.obsm["embeddings"]
  else:
    if args.num_hvgs != -1:
